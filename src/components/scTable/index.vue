	...
	export default {
		...
		methods: {
			...
			async getData(){
				this.loading = true;
				var reqData = {
					[config.request.token]: this.$TOOL.data.get("TOKEN"),
					[config.request.page]: JSON.stringify(this.currentPage-1),
					[config.request.pageSize]: JSON.stringify(this.pageSize),
